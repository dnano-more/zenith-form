...
export default function LoginPage() {
  ...
  // Fetch supported auth providers (e.g. Google OAuth if configured)
  ...
  const guestLoginMutation = trpc.auth.loginAsGuest.useMutation({
    ...
    onError: (err) => {
      toast.error(err.message || "Failed to log in as Guest");
              ...
              {isProvidersLoading ? (
                ...
                <a href={googleProvider.authUrl} className="block w-full">
                  <Button variant="outline" className="w-full h-11 font-medium gap-2 hover:bg-accent">
                    ...
                    <span>{googleProvider.displayText || "Sign in with Google"}</span>
                  </Button>
                </a>
              ) : null}
